...
import CardPreview, { NET_COLORS } from '../components/ui/CardPreview'
...
export default function AddCard() {
  ...
  const handleNext = () => {
    ...
    if (step === 2 && validateStep2()) { setStep(3); window.scrollTo(0,0) }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#080808', paddingTop:64 }}>
      <div style={{ maxWidth:1000, margin:'0 auto', padding:'56px 36px 80px' }}>
        ...
        <div style={{ marginBottom:48 }}>
          <button onClick={handleBack} style={{ background:'none', border:'none', color:'#333', cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', gap:6, marginBottom:32, fontFamily:'DM Sans,sans-serif', padding:0, transition:'color 0.15s' }}
          ...
          <h1 style={{ fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:'clamp(2rem,5vw,3.2rem)', letterSpacing:'-0.03em', lineHeight:1.05 }}>
            {TITLES[step]}
          </h1>
          <p style={{ fontSize:14, color:'#444', marginTop:10 }}>{SUBS[step]}</p>
        </div>
          ...
          <div className="ac-grid" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:60, alignItems:'start' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
              ...
              <div style={{ background:'#0a0a0a', border:'1px solid #111', borderRadius:12, padding:'14px 16px', display:'flex', gap:12, alignItems:'flex-start' }}>
                ...
                <p style={{ fontSize:12, color:'#2a2a2a', lineHeight:1.6 }}>
          ...
          <div className="ac-grid" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:60, alignItems:'start' }}>
            <div style={{ display:'flex', flexDirection:'column', gap:32 }}>
              ...
              <div>
                ...
                <p style={{ fontSize:13, color:'#444', marginBottom:16 }}>Select which deal categories your card offers advantages in.</p>
              ...
              <div>
                ...
                <p style={{ fontSize:13, color:'#444', marginBottom:16 }}>How you want your card listed in the network.</p>
                    ...
                    return (
                      <div key={tier.id} onClick={() => setForm(f => ({ ...f, tier: tier.id }))} style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize:12, color:'#333' }}>{tier.desc}</div>
                        </div>
                        <div style={{
              ...
              <div>
                ...
                <p style={{ fontSize:13, color:'#444', marginBottom:12 }}>Set a max monthly amount you're comfortable routing through this card.</p>
                <div style={{ position:'relative' }}>
                  <span style={{ position:'absolute', left:16, top:'50%', transform:'translateY(-50%)', color:'#2a2a2a', fontSize:14 }}>₹</span>
                  <input
                    type="number"
            ...
            <div style={{ paddingTop:8, position:'sticky', top:88 }}>
              ...
              <div style={{ marginTop:20, background:'#0c0c0c', border:'1px solid #1a1a1a', borderRadius:14, padding:16 }}>
                <div style={{ fontSize:10, color:'#2a2a2a', fontWeight:700, letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:10 }}>Listing summary</div>
                {form.categories.length > 0 && (
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:10 }}>
                    {form.categories.map(c => (
                      <span key={c} style={{ fontSize:10, background:'#141414', border:'1px solid #1e1e1e', borderRadius:5, padding:'2px 8px', color:'#444' }}>{c}</span>
                    ))}
                  </div>
                )}
                {form.tier && (
                  <div style={{ fontSize:12, color:'#333' }}>
                    Tier: <span style={{ color:'#555', fontWeight:600 }}>{CARD_TIERS.find(t=>t.id===form.tier)?.label}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        {step === 3 && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', paddingTop:20 }}>
            ...
            <div style={{ marginTop:40, maxWidth:440 }}>
              <h2 style={{ fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:'1.6rem', letterSpacing:'-0.02em', marginBottom:12 }}>
            ...
            <div style={{ marginTop:32, background:'#0c0c0c', border:'1px solid #1a1a1a', borderRadius:16, padding:'24px 28px', width:'100%', maxWidth:400, textAlign:'left' }}>
              {[
                ...
                <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', padding:'8px 0', borderBottom:'1px solid #0f0f0f' }}>
                  <span style={{ fontSize:12, color:'#333' }}>{row.label}</span>
                  <span style={{ fontSize:12, color:'#555', fontWeight:500, maxWidth:220, textAlign:'right' }}>{row.value}</span>
                </div>
              ))}
            </div>
